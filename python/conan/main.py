...
def fill_table(history, scores, table, parameter_dic, options):
    ...
    vec_model = [options.k, options.network_architecture, options.pool, 
...
def main():
    ...
    if options.y_variable in ["RCB_class", "ee_grade"]:
        ...
        columns = ['loss', 'mean_squared_error', 'val_loss', 
                   ...
                   'gaussian_noise', 'k', 'model', 'pooling', 'batch_size', 
                   'size', 'input_depth', 'fold_test', 'run_number']
    table = DataFrame(index=range(options.repeat*(options.n_fold-1)), columns=columns)

    options.run = 0
    for i in range(options.repeat):
        for j in range(options.n_fold - 1):
            options.run += 1
            ...
            predictions.to_csv("predictions_run_{}_test_fold_{}.csv".format(options.run, fold_test))
